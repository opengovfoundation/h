...
class window.DTM_MatcherCore
 ...
 with missing context!"
    ...
    prefixResult = @dmp.search @mapper.getCorpus(), prefix, expectedPrefixStart
    ...
    remainingText = @mapper.getCorpus().substr prefixEnd
    ...
    if pattern? and options.flexContext and not analysis.exact
      ...
      flexMatches = @pm.search @mapper.getCorpus()[prefixStart..suffixEnd], pattern
      delete candidate
      bestError = 2
...
#        errorLevel + ")"
  ...
  _search: (matcher, pattern, pos, options = {}) ->
    ...
    # Do the text search
    textMatches = matcher.search @mapper.getCorpus(), pattern, pos, options
    t2 = @timestamp()
  ...
  _analyzeMatch: (pattern, charRange, useFuzzy = false) ->
    ...
    found = @_normalizeString @mapper.getContentForCharRange charRange.start,
        charRange.end
    result =
      found: found
