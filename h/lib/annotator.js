...
(function() {
  ...
  Annotator = (function(_super) {
    ...
    Annotator.prototype._physicallyAnchorAnnotation = function(task) {
      var anchor, browserRange, highlight, mappings, range, serializedRange;
      if (task.target.physicalAnchor != null) {
        return;
      }
      anchor = task.target.virtualAnchor;
      mappings = this.domMapper.getMappingsForCharRange(anchor.start, anchor.end);
      ...
      task.annotation.ranges.push(serializedRange);
      highlight = this.highlightRange(range);
      $(highlight).data('annotation', task.annotation);
      $.merge(task.annotation.highlights, highlight);
      task.target.physicalAnchor = {
    ...
    Annotator.prototype._physicallyAnchorPage = function(index) {
      var task, tasks, _k, _len2, _results;
      if (!this.domMapper.isPageRendered(index)) {
        console.log("Page #" + index + " has not been rendered yet, so not anchoring.");
        return;
      }
      tasks = this.virtualAnchors[index];
      if (tasks == null) {
        return;
      }
      _results = [];
    ...
    Annotator.prototype._physicallyUnAnchorAnnotation = function(task) {
      var anchor, i, _ref1, _ref2;
      anchor = task.target.physicalAnchor;
      if (anchor == null) {
        return;
      }
      i = task.annotation.ranges.indexOf(anchor.range);
      [].splice.apply(task.annotation.ranges, [i, i - i + 1].concat(_ref1 = [])), _ref1;
      i = task.annotation.highlights.indexOf(anchor.highlight);
      [].splice.apply(task.annotation.highlights, [i, i - i + 1].concat(_ref2 = [])), _ref2;
      delete task.target.physicalAnchor;
      return this.publish('annotationPhysicallyUnAnchored', task);
    };
    ...
    Annotator.prototype._physicallyUnAnchorPage = function(index) {
      ...
      for (_k = 0, _len2 = tasks.length; _k < _len2; _k++) {
        task = tasks[_k];
        _results.push(this._physicallyUnAnchorAnnotation(task));
      }
      return _results;
    };
    ...
    Annotator.prototype._addVirtualAnchor = function(task) {
      var anchor, pageIndex, rendered, _base, _k, _ref1, _ref2,
        _this = this;
      task.taskIndex = this._taskIndex += 1;
      anchor = task.target.virtualAnchor;
      rendered = false;
      for (pageIndex = _k = _ref1 = anchor.startPage, _ref2 = anchor.endPage; _ref1 <= _ref2 ? _k <= _ref2 : _k >= _ref2; pageIndex = _ref1 <= _ref2 ? ++_k : --_k) {
        if ((_base = this.virtualAnchors)[pageIndex] == null) {
          _base[pageIndex] = [];
        }
        this.virtualAnchors[pageIndex].push(task);
        if (this.domMapper.isPageRendered(pageIndex)) {
          rendered = true;
        }
      }
      if (rendered) {
    ...
    Annotator.prototype._setupMatching = function() {
      ...
      strategies = [
        {
          ...
          init: function() {
            addEventListener("pdfPageMapped", function(evt) {
              return _this._physicallyAnchorPage(evt.pageIndex);
            });
            return addEventListener("pdfPageUnmapped", function(evt) {
      ...
      for (_k = 0, _len2 = strategies.length; _k < _len2; _k++) {
        ...
        if (s.mapper.applicable()) {
          ...
          this.virtualAnchoring = (_ref1 = this.domMapper.requiresVirtualAnchoring) != null ? _ref1 : false;
    ...
    Annotator.prototype.findAnchorWithTwoPhaseFuzzyMatching = function(target) {
      var browserRange, expectedEnd, expectedStart, match, normalizedRange, options, posSelector, prefix, quote, quoteSelector, result, suffix;
    ...
    Annotator.prototype.setupAnnotation = function(annotation) {
      var anchor, error, exception, normed, normedRanges, r, ranges, t, _k, _l, _len2, _len3, _ref1, _ref2;
