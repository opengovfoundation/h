...
(function() {
  ...
  Annotator = (function(_super) {
    ...
    Annotator.prototype._physicallyAnchorAnnotation = function(task) {
      var browserRange, mappings, range, selector;
      if (task.done) {
        return;
      }
      ...
      task.serializedRange = range.serialize(this.wrapper[0], '.annotator-hl');
      task.annotation.ranges.push(task.serializedRange);
      task.hl = this.highlightRange(range);
      $(task.hl).data('annotation', task.annotation);
      $.merge(task.annotation.highlights, task.hl);
      task.done = true;
    ...
    Annotator.prototype._physicallyUnAnchorAnnotation = function(task) {
      ...
      console.log("Should physically un-anchor:");
      console.log(task);
      i = task.annotation.ranges.indexOf(task.serializedRange);
      [].splice.apply(task.annotation.ranges, [i, i - i + 1].concat(_ref1 = [])), _ref1;
      delete task.serializedRange;
      i = task.annotation.highlights.indexOf(task.hl);
      ...
      strategies = [
        {
          ...
          init: function() {
            _this.domMapper.onPageReady = function(index) {
              return _this._physicallyAnchorPage(index);
            };
            return _this.domMapper.onPageDeleted = function(index) {
              return _this._physicallyUnAnchorPage(index);
            };
          }
        }, {
          name: "DOM generic",
