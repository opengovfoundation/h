...
(function() {
  ...
  Annotator = (function(_super) {
    ...
    Annotator.prototype.onPhysicallyAnchored = function(task) {};

    Annotator.prototype._physicallyAnchorAnnotation = function(task) {
      var browserRange, hl, mappings, range, selector;
      if (task.done) {
        return;
      }
      selector = task.anchor;
      mappings = this.docMapper.getMappingsForCharRange(selector.start, selector.end);
      browserRange = new Range.BrowserRange(mappings.realRange);
      range = browserRange.normalize(this.wrapper[0]);
      task.annotation.ranges.push(range.serialize(this.wrapper[0], '.annotator-hl'));
      hl = this.highlightRange(range);
      $(hl).data('annotation', task.annotation);
      $.merge(task.annotation.highlights, hl);
      task.done = true;
      return this.onPhysicallyAnchored(task);
    };

    Annotator.prototype._physicallyAnchorPage = function(index) {
      var anchors, task, _k, _len2, _results;
      if (!this.docMapper.isPageRendered(index)) {
        console.log("Page #" + index + " has not been rendered yet, so not anchoring.");
        return;
      }
      anchors = this.virtualAnchors[index];
      if (anchors == null) {
        return;
      }
      _results = [];
      for (_k = 0, _len2 = anchors.length; _k < _len2; _k++) {
        task = anchors[_k];
        _results.push(this._physicallyAnchorAnnotation(task));
      }
      return _results;
    };

    Annotator.prototype._taskIndex = 0;

    Annotator.prototype._addVirtualAnchor = function(pageIndex, task) {
      var _base,
        _this = this;
      task.taskIndex = this._taskIndex += 1;
      if ((_base = this.virtualAnchors)[pageIndex] == null) {
        _base[pageIndex] = [];
      }
      this.virtualAnchors[pageIndex].push(task);
      if (this.docMapper.isPageRendered(pageIndex)) {
        return setTimeout(function() {
          return _this._physicallyAnchorAnnotation(task);
        });
      }
    };

    Annotator.prototype.isPDF = function() {
      var _ref1;
      return (_ref1 = typeof PDFView !== "undefined" && PDFView !== null ? PDFView.initialized : void 0) != null ? _ref1 : false;
    };

    Annotator.prototype._setupMatching = function() {
      var _this = this;
      if (this.docMapper != null) {
        return;
      }
      if (this.isPDF()) {
        this.docMapper = this.pdfMapper = new PDFTextMapper();
        this.pdfMapper.onPageReady = function(index) {
          return _this._physicallyAnchorPage(index);
        };
        this.virtualAnchoring = true;
        this.virtualAnchors = {};
      } else {
        this.docMapper = this.domMapper = new DomTextMapper();
        this.domMatcher = new DomTextMatcher(this.domMapper);
        this.domMapper.setRootNode(this.wrapper[0]);
        this.virtualAnchoring = false;
      }
      return this;
    };

    Annotator.prototype._scan = function() {
      return this.pendingScan = this.docMapper.scan();
    };
    ...
    Annotator.prototype.getTextQuoteSelector = function(range) {
      ...
      startOffset = (this.docMapper.getInfoForNode(rangeStart)).start;
      rangeEnd = range.end;
      if (rangeEnd == null) {
        throw new Error("Called getTextQuoteSelector(range) on a range with no valid end.");
      }
      endOffset = (this.docMapper.getInfoForNode(rangeEnd)).end;
      quote = this.docMapper.getContentForCharRange(startOffset, endOffset);
      _ref1 = this.docMapper.getContextForCharRange(startOffset, endOffset), prefix = _ref1[0], suffix = _ref1[1];
      return selector = {
    ...
    Annotator.prototype.getTextPositionSelector = function(range) {
      ...
      startOffset = (this.docMapper.getInfoForNode(range.start)).start;
      endOffset = (this.docMapper.getInfoForNode(range.end)).end;
      return selector = {
    ...
    Annotator.prototype.findAnchorFromPositionSelector = function(target) {
      ...
      savedQuote = this.getQuoteForTarget(target);
      if (savedQuote != null) {
        content = this.docMapper.getContentForCharRange(selector.start, selector.end);
        currentQuote = this.normalizeString(content);
        if (currentQuote !== savedQuote) {
      ...
      if (this.virtualAnchoring) {
        return {
          startPage: this.docMapper.getPageIndexForPos(selector.start),
          endPage: this.docMapper.getPageIndexForPos(selector.end),
    ...
    Annotator.prototype.setupAnnotation = function(annotation) {
      var anchor, error, exception, index, normed, normedRanges, r, ranges, t, _k, _l, _len2, _len3, _m, _ref1, _ref2, _ref3, _ref4;
      ...
      for (_k = 0, _len2 = _ref1.length; _k < _len2; _k++) {
        ...
        try {
          ...
          if (anchor != null) {
            ...
            if (this.virtualAnchoring) {
              for (index = _l = _ref3 = anchor.startPage, _ref4 = anchor.endPage; _ref3 <= _ref4 ? _l <= _ref4 : _l >= _ref4; index = _ref3 <= _ref4 ? ++_l : --_l) {
