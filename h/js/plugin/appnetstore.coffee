...
class Annotator.Plugin.Store extends Annotator.Plugin
    ...
    if annotation not in @annotations
      ...
      this._apiRequest('create', annotation, ({data}) =>
        ...
        data =
          ...
          user: annotation.user  # keep the user in our format
        this.updateAnnotation annotation, data

        searchEndpoint = this._urlFor('search')
        options = this._apiRequestOptions('search', annotation)
        options.type = 'POST'
        $.ajax searchEndpoint,
          type: 'POST'
  ...
  loadAnnotationsFromSearch: (searchOptions) ->
    this._apiRequest 'search', {}, (data={}) =>
      annotations = []
      for d in data.data or []
        for a in (d.annotations or []) when a.type is 'is.hpt.a'
          for l in a.value.document.link when l.href is searchOptions.uri
            annotations.push a.value
            break
      data =
        rows: annotations
        total: annotations.length
      this._onLoadAnnotationsFromSearch data
  ...
  _dataFor: (annotation) ->
    ...
    # Max quote length: 256 (limit), 3 (line breaks), 2 (quotation marks)
    maxQuoteLength = 256 - 3 - 2 - title.length - text.length

    quote =
      switch
        when maxQuoteLength >= quote.length
          quote
        when maxQuoteLength >= 15
          quote.slice(0, maxQuoteLength-1) + '\u2026'
        else
          null
    ...
    if annotation.references?
      data.reply_to = annotation.references[annotation.references.length-1]
